...
def gerar_recorrencias_do_mes(mes_destino_str):
    ...
    baseado nos lançamentos marcados como recorrentes (recorrente = 1) no mês anterior.
    """
    ...
    recorrentes = [l for l in lancamentos_origem if l["recorrente"] == 1]
    ...
    chaves_existentes = set((e["descricao"], e["tipo"], e["esfera"], float(e["valor"])) for e in existentes_destino)

    novos_gerados = 0
    for orig in recorrentes:
        chave = (orig["descricao"], orig["tipo"], orig["esfera"], float(orig["valor"]))
        if chave in chaves_existentes:
            continue  # Já existe nesse mês, ignora

        # Monta a nova data de vencimento ajustando para o mês destino
        try:
